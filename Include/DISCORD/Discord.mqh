...
//dicord tocken api "_FG1ruQfOjiXvPjXDXKtfJMiZjtgjpkj"
//CLIENT ID 879201398057758731
...
enum ENUM_CHAT_ACTION
{
   ...
   ACTION_UPLOAD_VIDEO  ,   //sending video...
     ACTION_PREPARE_ORDER,  // prepare to send order
     ACTION_UPDATE_CHART,//update chart
     ACTION_TEST_MAX_TRADES//test maximum trades
};
//+------------------------------------------------------------------+
//|   ChatActionToString                                             |
//+------------------------------------------------------------------+
...
public:
   ...
   int               GetMe()
   {
      ...
      string url=StringFormat("%s/soauth2/sendPhoto",DISCORD_BASE_URL,m_token);
   ...
   int               GetUpdates()
   {
      ...
      string out;
      string url=StringFormat("%s/bot%s/getUpdates",DISCORD_BASE_URL,m_token);
      ...
      if(res==0)
      {
         ...
         for(int i=0; i<total; i++)
         {
            ...
            //---
            message_text[0]=item["message"]["text"].ToStr();
            msg.message_text=StringDecode(message_text[0]);
            
            printf("text"+message_text[0]);
            //---
            msg.from.id=item["message"]["from"]["id"].ToInt();

            msg.from_first_name=item["message"]["from"]["first_name"].ToStr();
            msg.from.first_name=StringDecode(msg.from_first_name);

            msg.from_last_name=item["message"]["from"]["last_name"].ToStr();
            msg.from.last_name=StringDecode(msg.from_last_name);

            msg.from.username=item["message"]["from"]["username"].ToStr();
            msg.from.username=StringDecode(msg.from_username);
            //---
            long chat_id=item["message"]["chat"]["id"].ToInt();
              msg.chat_id=chat_id;
              
            msg.chat.first_name=item["message"]["chat"]["first_name"].ToStr();
          
            msg.chat.last_name=item["message"]["chat"]["last_name"].ToStr();
         

            msg.chat_type=item["message"]["chat"]["type"].ToStr();
            
            msg.ok=ok;
            msg.sender_chat.id=item["message"]["sender_chat"]["id"].ToInt() ;
            msg.sender_chat.title=item["message"]["sender_chat"]["title"].ToStr();
            msg.sender_chat.type=item["message"]["sender_chat"]["type"].ToStr();
            msg.sender_chat.username=item["message"]["sender_chat"]["username"].ToStr();
       ...
       msg.update_id= item["update_id"].ToInt();
          msg.chat.first_name=item["message"]["chat"]["username"].ToStr();
            result.update_id=msg.update_id;
            ...
            if(m_users_filter.Total()==0 || (m_users_filter.Total()>0 && m_users_filter.SearchLinear(msg.from_username)>=0))
            {
               ...
               if(index==0)
               {
                  ...
                  chat.m_state=0;
                  chat.m_new_one.message_text=message_text[0];
                                    ...
                                    const ENUM_CHAT_ACTION _action)
   {
      if(m_token==NULL)
         ...
         return(ERR_TOKEN_ISEMPTY);
      string out;
      string url=StringFormat("%s/oauth2%s/sendChatAction",DISCORD_BASE_URL,m_token);
